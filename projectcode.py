...
# Assuming your data is stored in a DataFrame called 'X_encoded'
# You might need to install the datasketch library via pip if you haven't already
# pip install datasketch

# Initialize LSH
lsh = MinHashLSH(threshold=0.6, num_perm=128)

# Iterate over your dataset and hash each row
for idx, row in X_encoded.iterrows():
    # Initialize MinHash
    mh = MinHash(num_perm=128)
    
    # Update MinHash with True values
...
for i in range(299):
    # Get the query for the current index
    query = X_encoded.iloc[i]
    
    # Initialize MinHash for the query
    query_mh = MinHash(num_perm=128)

    # Update MinHash with True values for the query
    for col, value in query.items():
        if value:
            query_mh.update(col.encode('utf8'))

    # Query LSH for similar items
    similar_items = lsh.query(query_mh)
    print("Similar items for index", i, ":", similar_items)